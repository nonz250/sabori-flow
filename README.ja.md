...
sabori-flow ではオーケストレーション（TypeScript）と問題解決（AI）を分離しています。AI は Issue の理解とコーディングに集中し、それ以外は TypeScript スクリプトが担当します。
...
AI チャットアプリやデスクトップツールは、ファイル編集・コマンド実行・git 操作のたびに承認確認を求めます。自動処理したいのに、人間が張り付いて「許可」を押し続けるのでは本末転倒です。

sabori-flow は AI を CLI 経由（`claude -p --dangerously-skip-permissions`）で呼び出すので、ラベル付与から Pull Request 作成まで人間の操作は不要です。
