...
AI エージェントはパイプライン内の 1 つの CLI 呼び出しです。**Claude Code CLI** と **GitHub Copilot CLI** を実行エンジンとしてサポートしており、`config.yml` の `execution.engine` で選択できます。`-p` による非対話プロンプトを受け付ける CLI ベースの AI エージェントであれば、エンジンとして追加可能です。
...
- macOS
- Node.js v20+
- [Claude Code CLI](https://docs.anthropic.com/en/docs/claude-code) (`claude`) または [GitHub Copilot CLI](https://docs.github.com/en/copilot/reference/cli-command-reference) (`copilot`)
...
| `execution.engine` | 実行エンジン: `claude`（Claude Code CLI）、`copilot`（GitHub Copilot CLI）。デフォルトは `claude` |
...
このツールはデフォルトで `interactive` モードで選択された CLI エンジンを実行し、各操作にユーザー承認が必要です。完全自律実行を有効にするには `config.yml` で `execution.autonomy: full` を設定してください。これにより CLI に `--dangerously-skip-permissions`（Claude Code）または `--yolo`（Copilot）が付与され、マシン上でほぼ任意の操作が可能になります。launchd により定期的にユーザー操作なしで実行されます。
