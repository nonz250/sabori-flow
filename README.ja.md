...
"sabori" は日本語の「サボり」に由来しています。sabori-flow では**賢くサボる**ことを目指しています。退屈で定型的なタスクを AI に任せ、人間は本当に頭を使うべき仕事に集中する -- そんなワークフローを実現するツールです。

GitHub Issue にラベルを付けるだけで、sabori-flow がバックグラウンドで Issue を読み、方針を立て、コードを実装し、Pull Request を作成します。**ラベルを付けたら、あとは放置。**

sabori-flow は **AI プロダクトではなく、ワークフロープロダクト**です。重要なのは Issue 検出、ラベル駆動の状態遷移、隔離実行、構造化された出力というパイプライン設計であり、Issue を解く LLM が何であるかは実装の詳細に過ぎません。

## 設計思想

### スクリプトがオーケストレーションし、AI が課題を解く

sabori-flow は**決定的なオーケストレーション**と**知的な問題解決**を明確に分離しています。AI は Issue の理解とコーディングだけに集中し、それ以外は TypeScript スクリプトが担います -- 予測可能で、テスト可能で、ハルシネーションのリスクがありません。

| TypeScript スクリプト（決定的） | AI エージェント（知的） |
|---|---|
...
Issue の操作やラベルの管理は機械的な作業です。決定的なシステムが処理すべきであり、トークンを消費しハルシネーションのリスクがある知能に任せるべきではありません。

### CLI 実行による完全自動化

AI チャットアプリやデスクトップツールは、ファイル編集・ターミナルコマンド・git 操作のたびに承認確認を求めます。これでは完全な自動化は不可能です -- 人間が張り付いて「許可」をクリックし続ける必要があります。

sabori-flow は AI を CLI 経由（`claude -p --dangerously-skip-permissions`）で呼び出し、ラベル付与から Pull Request 作成まで**人間の介入ゼロ**で実行します。ダイアログなし、承認なし、見守り不要。

### LLM 非依存のアーキテクチャ

AI エージェントはパイプライン内の 1 つの CLI 呼び出しに過ぎません。現在は Claude Code CLI を使っていますが、アーキテクチャは特定のプロバイダに依存しません。OpenAI Codex、GitHub Copilot CLI など、CLI ベースの AI エージェントであれば差し替え可能です。*（マルチエンジン対応は計画中ですが未実装です。）*

重要なのはワークフロー設計であり、どの LLM を組み込むかではありません。

### 最先端ではなく、現実的なフロー

AI エージェントのオーケストレーション -- マルチエージェント連携、思考チェーンルーティング、ツール使用ループ -- は最先端技術であり、大きな可能性を秘めています。しかし多くのチームにとって今すぐ効果があるのは、もっとシンプルなことです: **今あるワークフローに AI を合流させる。**

あなたのチームはすでに GitHub Issue を書き、PR をレビューし、ラベルを使っています。sabori-flow は新しいパラダイムの採用を求めません。今やっていることの「真ん中」を自動化するだけです:

```
[Issue を書く] → [ラベルを付ける] → [sabori-flow] → [PR をレビュー] → [マージ]
```

これは未来のビジョンではなく、チームが今まさにそうやって働いている現実のフローです。
...
- **Issue 駆動ではない自動化**（例: 日次サマリー、Slack 通知）。
